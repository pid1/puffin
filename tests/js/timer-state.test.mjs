...
test("a well-formed timer round-trips", () => {
	const state = {
		...
		childId: 3,
		segments: [
			{ side: "breast_left", startTime: "2026-07-19T10:00:00Z", endTime: null },
		],
	};
...
test("corrupt JSON is discarded, not thrown", () => {
	const app = loadApp({
		localStorage: { [TIMER_KEY]: '{"active":true,"segments":[{"side"' },
	});
...
test("the legacy single-side format migrates to segments", () => {
	...
	const app = loadApp({
		localStorage: { [TIMER_KEY]: JSON.stringify(legacy) },
	});
