...
test("refreshDashboard advances a page left open past midnight", () => {
	...
	assert.equal(
		app.document.getElementById("cal-date-label").textContent,
		"Today",
	);
	assert.equal(app.document.getElementById("cal-next").disabled, true);
	assert.equal(
		app.document.getElementById("cal-today-pill").classList.contains("hidden"),
		true,
	);
});
...
test("refreshDashboard leaves a deliberately-viewed past day alone", () => {
	...
	assert.equal(
		app.state.currentDate.getUTCDate(),
		viewed.getUTCDate(),
		"stays on the viewed day",
	);
});
...
test("loadChildren keeps last-known state when the fetch fails", async () => {
	...
	assert.ok(
		app.consoleErrors.some((e) => e.includes("Failed to load children")),
	);
});
