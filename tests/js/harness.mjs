...
export function loadApp(opts = {}) {
	const source = SOURCE_FILES.map((name) =>
		readFileSync(join(STATIC_JS, name), "utf8"),
	).join("\n");
	...
	const fetchStub =
		opts.fetch || (() => Promise.reject(new Error("fetch not stubbed")));
	...
	const epilogue = `
        ;(function () {
            ...
            ${EXPOSED_STATE.map(
							(
								n,
							) => `Object.defineProperty(__exports.state, ${JSON.stringify(n)}, {
            ...
            ${OVERRIDABLE.map(
							(n) =>
								`__exports.override[${JSON.stringify(n)}] = (f) => { ${n} = f; };`,
						).join("\n")}
        })();
    `;
