...
const QUICK_ADD_TYPES = [
	{
		...
		label: "Diaper",
		modal: "diaper-modal",
		cardClass: "card-diaper",
	},
	{
		key: "feeding",
		label: "Feeding",
		modal: "feeding-modal",
		cardClass: "card-feeding",
	},
	{
		key: "health",
		label: "Health",
...
function resolveSelectedChild() {
	...
	if (typeof stored === "number" && childProfiles.some((c) => c.id === stored))
		return stored;
...
function renderChildHeader() {
	...
	if (showBar) {
		...
		document.getElementById("unassigned-assign-target").innerHTML =
			childOptionsHtml();
	}
}
...
function offerMigration(child, count) {
	...
	showConfirm({
		...
		body:
			`You have ${count} care log${plural} that aren't linked to any child. ` +
			`Move them all to ${child.name}?`,
		note:
			"This bulk move is only offered once, when you create your first child. " +
...
function confirmMigration(child, count) {
	...
	showConfirm({
		title: "Confirm move",
		body:
			`This will assign all ${count} existing log${plural} to ${child.name}. ` +
...
async function runBulkAssign(child) {
	try {
		const result = await api.post(
			`/api/children/${child.id}/assign-unassigned`,
		);
		closeModal("confirm-modal");
		await refreshChildUI();
...
function renderChildList() {
	...
	container.innerHTML = childProfiles
		.map(
			(c) => `
...
function persistQuickAddToggle(childId, changedBox) {
	...
	setEnabledQuickAddTypes(
		childId,
		checked.map((b) => b.value),
	);
	boxes.forEach((b) => {
		b.disabled = b.checked && checked.length === 1;
	});
}
...
function confirmChildDelete(childId) {
	...
	showConfirm({
		...
		body:
			`${child.name}'s logs will be kept and moved to "Unassigned logs", ` +
...
function initChildSettings() {
	...
	document.getElementById("child-list").addEventListener("click", (e) => {
		...
		if (!btn) return;
		if (btn.dataset.childRename)
			startChildRename(parseInt(btn.dataset.childRename, 10));
		else if (btn.dataset.childRenameSave)
			saveChildRename(parseInt(btn.dataset.childRenameSave, 10));
		else if (btn.dataset.childRenameCancel) renderChildList();
		else if (btn.dataset.childDelete)
			confirmChildDelete(parseInt(btn.dataset.childDelete, 10));
	});
